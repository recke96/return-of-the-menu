...
import { type MenuItem, MenuItemSchema } from "./schema.ts";
...
export type SaiCookartLoaderConfig = SaiCookartLoaderRequiredConfig &
  Partial<SaiCookartLoaderOptionalConfig>;

export const DefaultSaiCookartLoaderOptions = {
  apiEndpoint: "https://api.sai-cookart.at/foods",
} as SaiCookartLoaderOptionalConfig;

export async function loadSaiCookartMenu(
  options: SaiCookartLoaderConfig,
): Promise<ReadonlyArray<MenuItem>> {
  const { apiEndpoint, logger } = Object.assign(
    {},
    DefaultSaiCookartLoaderOptions,
    options,
  );
...
async function* fetchRestaurantData(
  ...
  logger.debug(
    `fetchRestaurantData - Result ${response.status} ${response.statusText}:\n${JSON.stringify(data)}`,
  );
